      ...
      <div className="min-h-screen flex items-center justify-center">
        <form
          ...
          <h1 className="text-white text-3xl mt-10 font-medium">
            {state === "login" ? t('login.titleLogin') : t('login.titleRegister')}
          </h1>
          ...
          {state !== "login" && (
            <div className="flex items-center mt-6 w-full bg-white/5 ring-2 ring-white/10 focus-within:ring-red-500/60 h-12 rounded-full overflow-hidden pl-6 gap-2 transition-all ">
              ...
              <input type="text" name="name" placeholder={t('login.namePlaceholder')} className="w-full bg-transparent text-white placeholder-white/60 border-none outline-none " value={formData.name} onChange={handleChange} required />
            </div>
          )}
          ...
          <div className="flex items-center w-full mt-4 bg-white/5 ring-2 ring-white/10 focus-within:ring-red-500/60 h-12 rounded-full overflow-hidden pl-6 gap-2 transition-all ">
            ...
            <input type="email" name="email" placeholder={t('login.emailPlaceholder')} className="w-full bg-transparent text-white placeholder-white/60 border-none outline-none " value={formData.email} onChange={handleChange} required />
          </div>

          <div className=" flex items-center mt-4 w-full bg-white/5 ring-2 ring-white/10 focus-within:ring-red-500/60 h-12 rounded-full overflow-hidden pl-6 gap-2 transition-all ">
            ...
            <input type="password" name="password" placeholder={t('login.passwordPlaceholder')} className="w-full bg-transparent text-white placeholder-white/60 border-none outline-none" value={formData.password} onChange={handleChange} required />
          </div>
          ...
          <button type="submit" className="mt-2 w-full h-11 rounded-full text-white bg-red-600 hover:bg-red-500 transition " >
            {state === "login" ? t('login.loginButton') : t('login.registerButton')}
          </button>

          <p onClick={() => setState(prev => prev === "login" ? "register" : "login")} className="text-gray-400 text-sm mt-3 mb-11 cursor-pointer" >
            {state === "login" ? t('login.switchToRegister') : t('login.switchToLogin')}
            <span className="text-red-400 hover:underline ml-1">{t('login.switchLink')}</span>
          </p>
        </form>
      </div>
    </>
