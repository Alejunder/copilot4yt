        ...
        <div id="pricing" className="px-4 md:px-16 lg:px-24 xl:px-32">
            ...
            <div className="flex flex-wrap items-center justify-center gap-8 mt-20">
                {pricingData.map((plan: IPricing, index: number) => (
                    ...
                    >
                        {plan.mostPopular && (
                            <p className="absolute px-3 text-sm -top-3.5 left-3.5 py-1 bg-red-400 rounded-full">Most Popular</p>
                        )}
                        ...
                        <ul className="list-none text-slate-300 mt-6 space-y-2">
                            {plan.features.map((feature, index) => (
                                <li key={index} className="flex items-center gap-2">
                                    <CheckIcon className="size-4.5 text-red-600" />
                                    <p>{feature}</p>
                                </li>
                            ))}
                        </ul>
                        <button type="button" className={`w-full py-2.5 rounded-md font-medium mt-7 transition-all ${plan.mostPopular ? 'bg-white text-red-600 hover:bg-slate-200' : 'bg-red-500 hover:bg-red-600'}`}>
                            Get Started
                        </button>
                    </motion.div>
                ))}
